...
import {
  DAO,
  NTGovernanceERC20Mock,
  NTGovernanceERC20Mock__factory,
...
} from '../typechain-types/contracts/NTTokenVoting';
import {ExecutedEvent} from '../typechain-types/@aragon/osx/core/dao/DAO';
...
const majorityVotingBaseInterface = new ethers.utils.Interface([
...
/* describe('TokenVoting', function () {
  ...
  let voting: NTTokenVoting;
  let dao: DAO;
  let ntgovernanceErc20Mock: NTGovernanceERC20Mock;
  let NTGovernanceERC20Mock: NTGovernanceERC20Mock__factory;
  ...
  beforeEach(async function () {
    ...
    NTGovernanceERC20Mock = new NTGovernanceERC20Mock__factory(signers[0]);
    ntgovernanceErc20Mock = await NTGovernanceERC20Mock.deploy(
      dao.address,
      'GOV',
      'GOV',
      {
  ...
  async function setBalances(
    ...
    const promises = balances.map(balance =>
      ntgovernanceErc20Mock.setBalance(balance.receiver, balance.amount)
    );
  ...
  async function setTotalSupply(totalSupply: number) {
    ...
    const currentTotalSupply: BigNumber =
      await ntgovernanceErc20Mock.getPastTotalSupply(block.number - 1);

    await ntgovernanceErc20Mock.setBalance(
  ...
  describe('initialize: ', async () => {
    ...
    it('emits the `MembershipContractAnnounced` event', async () => {
      await expect(
        ...
        .withArgs(ntgovernanceErc20Mock.address);
    });
  ...
  describe('isMember: ', async () => {
    it('returns true if the account currently owns at least one token', async () => {
      ...
      expect(await ntgovernanceErc20Mock.balanceOf(signers[0].address)).to.eq(1);
      expect(await ntgovernanceErc20Mock.balanceOf(signers[1].address)).to.eq(0);

      expect(await ntgovernanceErc20Mock.getVotes(signers[0].address)).to.eq(1);
      expect(await ntgovernanceErc20Mock.getVotes(signers[1].address)).to.eq(0);
    ...
    it('returns true if the account currently has one at least one token delegated to her/him', async () => {
      ...
      await ntgovernanceErc20Mock
        .connect(signers[0])
        .delegate(signers[1].address);

      expect(await ntgovernanceErc20Mock.getVotes(signers[0].address)).to.eq(0);
      expect(await ntgovernanceErc20Mock.getVotes(signers[1].address)).to.eq(1);
  ...
  describe('Proposal creation', async () => {
    ...
    context('minProposerVotingPower > 0', async () => {
      ...
      it('reverts if `_msgSender` owns no tokens and has no tokens delegated to her/him in the current block although having them in the last block', async () => {
        ...
        const tx1 = await ntgovernanceErc20Mock
          .connect(signers[0])
        ...
        expect(
          await ntgovernanceErc20Mock.balanceOf(signers[0].address)
        ).to.equal(votingSettings.minProposerVotingPower);
        expect(
          await ntgovernanceErc20Mock.balanceOf(signers[1].address)
        ).to.equal(0);
